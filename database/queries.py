class SQLQueries:
    ...
    TABLES = {
        ...
        "clients": {"id": "client_id",
                    "cols": ["full_name", "phone_primary", "phone_secondary", "email", "birth_date", "address",
                             "registration_date", "medical_notes", "photo_path"], "display_col": "full_name"},
        "staff": {"id": "staff_id",
    ...
    JOINED_QUERIES = {
        "equipment": "SELECT e.equipment_id, z.name, e.name, e.purchase_date, e.last_service_date, e.status FROM equipment e JOIN zones z ON e.zone_id = z.zone_id",
        "client_subscriptions": "SELECT s.subscription_id, c.full_name, t.title, s.start_date, s.end_date, s.remaining_freeze_days, s.is_blocked FROM client_subscriptions s JOIN clients c ON s.client_id = c.client_id JOIN membership_types t ON s.type_id = t.type_id",
        "schedule": "SELECT sch.schedule_id, cl.name, st.full_name, z.name, sch.start_time, sch.end_time FROM schedule sch JOIN classes cl ON sch.class_type_id = cl.class_type_id JOIN staff st ON sch.coach_id = st.staff_id JOIN zones z ON sch.zone_id = z.zone_id",
        "class_registrations": "SELECT r.registration_id, (cl.name || ' | ' || sch.start_time), c.full_name, r.registration_time, r.status FROM class_registrations r JOIN schedule sch ON r.schedule_id = sch.schedule_id JOIN classes cl ON sch.class_type_id = cl.class_type_id JOIN clients c ON r.client_id = c.client_id",
        "attendance_log": "SELECT a.visit_id, c.full_name, a.entry_dt, a.exit_dt FROM attendance_log a JOIN clients c ON a.client_id = c.client_id",
        "payments": "SELECT p.payment_id, c.full_name, p.amount, p.payment_date, p.payment_method FROM payments p JOIN clients c ON p.client_id = c.client_id"
    }
